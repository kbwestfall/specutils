...
class SpectrumList(list, NDIOMixin):
    ...
    @property
    def labels(self) -> Optional[dict[str] | list]:
        return self._id_map or self._lazy_labels
    ...
    def _resolve_key(self, key: str) -> int:
        ...
        # if lazy labels but not id map, set the mapping
        if not self._id_map and self._lazy_labels:
            self.set_id_map(dict(zip(self._lazy_labels, range(len(self._lazy_labels)))))
